...
import { DEFAULT_EXCHANGE_RATE, tshToUsd } from "../utils/currency";
import {
  normalizeStatus,
  isConfirmedStatus,
  isCancelledStatus,
  ...
  getConfirmationStatusBucket,
  getShippingStatusBucket,
} from "../config/statusMapping";
import {
  generateProductMappingCode,
  mapCampaignSpendToProduct,
} from "../utils/adsMapping";
import { calculateLandedCostPerUnit } from "../utils/stockCalculations";
...
export const PAGE_DATE_FILTER_PRESETS = [
...
export function getDateRangeFromPreset(preset, todayString = getTodayString()) {
  if (preset === "today") return { start: todayString, end: todayString };
  if (preset === "yesterday") {
    const yesterday = addDaysToDateString(todayString, -1);
    return { start: yesterday, end: yesterday };
  }
  if (preset === "last7Days") {
    return { start: addDaysToDateString(todayString, -6), end: todayString };
  }
  if (preset === "last30Days") {
    return { start: addDaysToDateString(todayString, -29), end: todayString };
  }

  return { start: "", end: "" };
}

export function resolveDateRangeFilter(filter = {}, todayString = getTodayString()) {
  const preset = String(filter?.preset || "last30Days");
  if (preset !== "custom") {
    return { preset, ...getDateRangeFromPreset(preset, todayString) };
  }

  const start = String(filter?.startDate || "");
  const end = String(filter?.endDate || "");
  if (!start && !end) return { preset: "custom", start: "", end: "" };
  if (!start) return { preset: "custom", start: end, end };
  if (!end) return { preset: "custom", start, end: start };
  if (end < start) return { preset: "custom", start: end, end: start };
...
export function doesDateRangeOverlap(startA, endA, startB, endB) {
  const normalizedStartA = String(startA || "");
  const normalizedEndA = String(endA || normalizedStartA);
  const normalizedStartB = String(startB || "");
  const normalizedEndB = String(endB || normalizedStartB);
...
export function buildMappedMetaRows(rows, products, campaignMappings = {}) {
  const parseCampaignNameMetadata = (campaignName) => {
    const rawName = String(campaignName || "").trim();
    const parts = rawName.split("|").map((part) => part.trim()).filter(Boolean);
    const market = parts[0] || "";
    const productCode = parts[1] || "";
    const audience = parts[2] || "";
    const phaseCandidate = String(parts[3] || "").toUpperCase();
    const versionCandidate = String(parts[4] || "").toUpperCase();
    const phase = phaseCandidate === "TEST" || phaseCandidate === "SCALE" ? phaseCandidate : "";
    const version = /^V\d+$/i.test(versionCandidate) ? versionCandidate : "";
    const parsed = parts.length >= 5 && Boolean(productCode) && Boolean(phase) && Boolean(version);
...
export function getCustomerOrderTotalTzs(customer, product) {
  return getOrderRevenueAmounts(customer, product, USD_TO_TZS).revenueTsh;
}
...
export function getDefaultSituationData() {
  return {
    ...
    fixedCharges: [],
    adInputs: {},
    productWinnerThresholdTzs: 0,
    productAlertThresholds: {
      minStockQuantity: 20,
      minDeliveryRatePct: 45,
      highAdsSpendTzs: 250000,
      lowDeliveredOrders: 5,
    },
    hourlyAdsSnapshots: [],
        ...
        ? Object.fromEntries(
            Object.entries(value.adInputs).map(([productId, entry]) => [
              productId,
              {
                ...
                manualAdsSpendTzs: Math.max(0, parseLooseNumber(entry?.manualAdsSpendTzs)),
                leads: Math.max(0, Math.round(parseLooseNumber(entry?.leads))),
                confirmedOrders: Math.max(0, Math.round(parseLooseNumber(entry?.confirmedOrders))),
                deliveredOrders: Math.max(0, Math.round(parseLooseNumber(entry?.deliveredOrders))),
              },
            ])
          )
        : defaults.adInputs,
    productWinnerThresholdTzs: Math.max(0, parseLooseNumber(value?.productWinnerThresholdTzs)),
    productAlertThresholds:
      value?.productAlertThresholds && typeof value.productAlertThresholds === "object"
        ? {
            minStockQuantity: Math.max(
              0,
              Math.round(parseLooseNumber(value.productAlertThresholds?.minStockQuantity))
            ),
            minDeliveryRatePct: Math.max(
              0,
              Math.min(100, parseLooseNumber(value.productAlertThresholds?.minDeliveryRatePct))
            ),
            highAdsSpendTzs: Math.max(
              0,
              parseLooseNumber(value.productAlertThresholds?.highAdsSpendTzs)
            ),
            lowDeliveredOrders: Math.max(
              0,
              Math.round(parseLooseNumber(value.productAlertThresholds?.lowDeliveredOrders))
            ),
          }
        : defaults.productAlertThresholds,
...
export function sanitizeMetaAdsState(value) {
  ...
  const lifetimeSpendTzs = Math.max(0, parseLooseNumber(value?.lifetimeSpendTzs));
  const fallbackCumulativeSpendTzs = lifetimeSpendTzs;
  const explicitCumulativeTrackedSpendTzs = Math.max(0, parseLooseNumber(value?.cumulativeTrackedSpendTzs));
  const normalizedCumulativeTrackedSpendTzs =
    lifetimeSpendTzs > 0
      ? lifetimeSpendTzs
      : explicitCumulativeTrackedSpendTzs > 0
        ? explicitCumulativeTrackedSpendTzs
        : fallbackCumulativeSpendTzs;
  ...
  return {
    ...
    campaignMetadata: Array.isArray(value?.campaignMetadata)
      ? value.campaignMetadata.map((entry) => ({
          campaign_name: String(entry?.campaign_name || ""),
          product_code: String(entry?.product_code || ""),
          phase: String(entry?.phase || ""),
          version: String(entry?.version || ""),
          created_at: entry?.created_at || null,
          updated_at: entry?.updated_at || null,
        }))
      : defaults.campaignMetadata,
    ...
    savedCampaigns: Array.isArray(value?.savedCampaigns) ? value.savedCampaigns : defaults.savedCampaigns,
    dailySpendSnapshots,
    ...
    cumulativeTrackedSpendTzs: normalizedCumulativeTrackedSpendTzs,
    lifetimeSpendTzs,
...
export function getConfirmationStatusRule(status) {
  ...
  if (normalized === "confirmed" || normalized.startsWith("confirmed-")) {
    return CONFIRMATION_STATUS_RULES.confirmed;
  }
  if (normalized === "duplicate" || normalized.startsWith("duplicate-")) {
    return CONFIRMATION_STATUS_RULES.duplicate;
  }
  return CONFIRMATION_STATUS_RULES[normalized] || null;
}
...
export function getConfirmationBucket(status) {
  return getConfirmationStatusBucket(status);
}

export function getShippingBucket(status) {
  return getShippingStatusBucket(status);
}
...
export function sanitizeCustomerRecord(customer) {
  ...
  const parsedOrderDate = parseDateInput(customer?.orderDate) || new Date();
  const normalizedOrderDate = formatDateInput(parsedOrderDate);
  ...
  return {
    ...
    amount_tsh: Math.max(0, parseLooseNumber(customer?.amount_tsh ?? customer?.amountTsh)),
    amount_usd: Math.max(0, parseLooseNumber(customer?.amount_usd ?? customer?.amountUsd)),
    sourceOrderId: customer?.sourceOrderId || null,
    order_id: customer?.order_id || customer?.sourceOrderId || null,
    import_key: customer?.import_key || null,
    importSource: customer?.importSource || null,
    lastImportedAt: normalizeIsoLike(customer?.lastImportedAt),
    lastShippingImportedAt: normalizeIsoLike(customer?.lastShippingImportedAt),
    updatedAt: normalizeIsoLike(customer?.updatedAt || customer?.updated_at),
    confirmation_updated_at: normalizeIsoLike(customer?.confirmation_updated_at || customer?.confirmationUpdatedAt),
    assignedTo: String(customer?.assignedTo || ""),
    normalized_phone: normalizePhoneValue(customer?.normalized_phone || customer?.phone),
    product_ref: String(customer?.product_ref || customer?.productRef || ""),
    raw_row_data: customer?.raw_row_data && typeof customer.raw_row_data === "object" ? customer.raw_row_data : null,
    extra_fields: customer?.extra_fields && typeof customer.extra_fields === "object" ? customer.extra_fields : {},
    line_item_index: Math.max(0, Number(customer?.line_item_index || 0)),
    multi_product_revenue_allocated: Boolean(customer?.multi_product_revenue_allocated),
    confirmation_status_raw: String(customer?.confirmation_status_raw || customer?.confirmationStatus || ""),
    shipping_status_raw: String(customer?.shipping_status_raw || customer?.shippingStatus || ""),
...
export function isCountableLeadForService(status) {
  return !isInvalidLeadStatus(status);
}
...
export function getUnitProductCostTzs(product) {
  return getUnitProductCostUSD(product) * USD_TO_TZS;
}

export function getDeliveryCostPerUnitTzs(product) {
  return Math.max(0, parseLooseNumber(product?.delivery));
}

export function calculateProductPerformance(product, values = {}) {
  const unitsSold = Math.max(0, Number(values.unitsSold || 0));
  const totalRevenue = Math.max(0, Number(values.totalRevenue || 0));
  const totalAdsSpend = Math.max(0, Number(values.totalAdsSpend || 0));
  const costPerUnitUsd = getUnitProductCostUSD(product);
  const costPerUnitTzs = getUnitProductCostTzs(product);
  const deliveryCostPerUnitTzs =
    Number(values.totalServiceFeeTzs || 0) > 0 && unitsSold > 0
      ? Number(values.totalServiceFeeTzs) / unitsSold
      : getDeliveryCostPerUnitTzs(product);
  const totalProductCost = unitsSold * costPerUnitTzs;
  const totalDeliveryCost =
    Number(values.totalServiceFeeTzs || 0) > 0
      ? Math.max(0, Number(values.totalServiceFeeTzs))
      : unitsSold * deliveryCostPerUnitTzs;
  const profit = totalRevenue - totalAdsSpend - totalProductCost - totalDeliveryCost;
...
export function calculateCodFunnelMetrics(values = {}) {
  const adsSpend = Math.max(0, parseLooseNumber(values?.adsSpend));
  const leads = Math.max(0, Math.round(parseLooseNumber(values?.leads)));
  const confirmedOrders = Math.max(0, Math.round(parseLooseNumber(values?.confirmedOrders)));
  const deliveredOrders = Math.max(0, Math.round(parseLooseNumber(values?.deliveredOrders)));
  ...
  return {
    ...
    confirmedOrders,
    deliveredOrders,
    cpa: deliveredOrders > 0 ? adsSpend / deliveredOrders : 0,
    confirmationRate: leads > 0 ? (confirmedOrders / leads) * 100 : 0,
    deliveryRate: confirmedOrders > 0 ? (deliveredOrders / confirmedOrders) * 100 : 0,
  };
}

export function calculateProductAlerts(values = {}, thresholds = {}) {
  const resolvedThresholds = sanitizeSituationData({
    productAlertThresholds: thresholds,
  }).productAlertThresholds;
  const profit = Number(values?.profit || 0);
  const stockQuantity = Math.max(0, Number(values?.stockQuantity || 0));
  const adsSpend = Math.max(0, Number(values?.adsSpend || 0));
  const leads = Math.max(0, Number(values?.leads || 0));
  const confirmedOrders = Math.max(0, Number(values?.confirmedOrders || 0));
  const deliveredOrders = Math.max(0, Number(values?.deliveredOrders || 0));
  const deliveryRate = Math.max(0, Number(values?.deliveryRate || 0));
  ...
  if (profit < 0) {
    alerts.push({
      key: "profit",
      tone: "danger",
      message: "This product is losing money.",
    });
  }

  if (stockQuantity < resolvedThresholds.minStockQuantity) {
    alerts.push({
      key: "stock",
      tone: "warning",
  ...
  if (confirmedOrders > 0 && deliveryRate < resolvedThresholds.minDeliveryRatePct) {
    alerts.push({
      key: "delivery",
      tone: "warning",
      message: "Low delivery rate warning.",
    });
  }

  if (
    adsSpend >= resolvedThresholds.highAdsSpendTzs &&
    deliveredOrders < resolvedThresholds.lowDeliveredOrders &&
    (leads > 0 || confirmedOrders > 0 || adsSpend > 0)
  ) {
    alerts.push({
      key: "ads",
      tone: "danger",
      message: "High ads spend with low delivery.",
    });
  }
